...
int main(int argc, char * argv[])
{
   ...
   FILE* outputFile;
	...
	for(int k=0;k<faults.size(); k++){
	   fprintf(outputFile, "%d %d\n", faults[k]>>1, faults[k]%2);
	}
	detected_fault.clear();
    fclose(outputFile);
	
	cout<<"Preprocess: "<< argv[1]<< endl;
	cout<<"============================="<<endl;
	cout<<"Full fault list size\t Collapse fulat list size\t\n" <<endl;
	cout<<Nnodes*2<<"\t\t\t" << faults.size()<< "\t"<<endl;
	cout<<"============================="<<endl;
...
bool Dalg(int level) {
	...
	Dalg_count++;
	if(Dalg_count> 100000) return false;
	...
	if(!DatOut) {
					if(Dfront.size()==0) { //cout<<" Dfront size=0"<<endl; unassign(level-1); 
					return false;}
					else  { 
						int iter_while= 0;
			...
			for(int j=0;j< np->fin ;j++){
			    if(Node[np->unodes[j]].value==2){
					if(np->type==3 || np->type==4){
						...
						Node[np->unodes[j]].assign_level= level+1;						
					}
...
bool DalgCall( int fault){
	...
	if(fault%2==0)				    {  np->value= 3; if(np->type==1) Node[np->unodes[0]].value=1; }//D 
	else 						    {  np->value= 4; if(np->type==1) Node[np->unodes[0]].value=0;}//D 
